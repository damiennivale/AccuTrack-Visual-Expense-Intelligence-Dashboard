...
def run_ocr(image_path):
    """Run OCR with preprocessing for better accuracy"""
    from preprocess import document_unwarping, document_preprocess, document_OCR
    import cv2

    unwarped = document_unwarping([image_path], temp_dir=TEMP_DIR)
    if not unwarped:
        return None, "Unwarping failed"

    preprocessed = document_preprocess(unwarped, temp_dir=TEMP_DIR)
    if not preprocessed:
        return None, "Preprocessing failed"

    json_paths = document_OCR(preprocessed, temp_dir=TEMP_DIR)
    if not json_paths:
    ...
    blur_warning = None
    if blur_score < 100:
        blur_warning = f"Image quality low (blur: {blur_score:.1f})"
...
def parse_with_llm(ocr_text):
    ...
    return call_openrouter(ocr_text, debug_path=os.path.join(REPO, "response.json"))
...
def process_receipt(image_path):
    ...
    #Step 3: Save to DB
    receipt_for_db = copy.deepcopy(receipt)
    receipt_id = save_to_db(receipt_for_db, image_path)
